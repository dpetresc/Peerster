...
// GUI
// Send a file download request
function sendDownloadArgs(origin, metahash, fileName) {
    if (fileName != "" && metahash != "") {
        $.ajax({
            type: "POST",
            url: "http://localhost:8080/file",
            data: {
    ...
    $.ajax({
        ...
        success: function (data, status) {
            ...
            for (let match of data) {
                ...
                matchDiv.ondblclick = (function() {
                    var currentFileName = fileName;
                    var currentMetaHash = metahash;
                    var currentOrigin = origin
                    return function() { 
                        sendDownloadArgs(currentOrigin, currentMetaHash, currentFileName)
                    }
                })()
...
setInterval(getMatches, 700);

var confirmedMessage = new Set();
...
setInterval(getConfirmationMessages, 700);
// END GUI